...
class AuthorityTest {
    ...
    void mapsRoleToLandingPath(Authority authority, String expected) {
        assertThat(authority.landingPath()).isEqualTo(expected);
    }

    @ParameterizedTest
    @EnumSource(Authority.class)
    @DisplayName("모든 역할이 착지 경로를 가진다 — 역할을 추가하고 매핑을 잊으면 로그인 직후 갈 곳이 없다")
    void everyRoleHasLandingPath(Authority authority) {
        assertThat(authority.landingPath()).startsWith("/");
    }
}
