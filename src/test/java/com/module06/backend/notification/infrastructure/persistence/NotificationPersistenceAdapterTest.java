 ...
 *
 * 클래스에 @Transactional을 안 건다 — saveIfAbsent()가 이제 REQUIRES_NEW로 항상 실제 커밋을
 * 한다(CodeRabbit 지적: AFTER_COMMIT 리스너에서 호출되므로 독립 트랜잭션이 필요). 테스트
 * 트랜잭션 롤백으로는 이 커밋을 되돌릴 수 없으므로, @AfterEach에서 직접 지운다.
 */
@SpringBootTest
