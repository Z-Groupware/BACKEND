...
/* MEET-06 익명 요청이 AuthPrincipal 인자 해석 전에 공통 보안 필터에서 차단되는지 검증한다. */
@DisplayName("MEET-06 회의 취소 보안")
@SpringBootTest
@AutoConfigureMockMvc
class MeetingCancellationSecurityTest {

    /* 실제 SecurityFilterChain과 MVC 인자 해석을 통과하는 HTTP 요청 실행기다. */
    @Autowired
    private MockMvc mockMvc;

    /* 인증 실패 요청이 회의 취소 비즈니스 로직에 도달하지 않는지 확인하는 대역이다. */
    @MockitoBean
    private CancelMeetingUseCase cancelMeetingUseCase;

    /* Access Token 없는 DELETE 요청이 Controller 전에 공통 401로 차단되는지 검증한다. */
    @Test
    ...
    void rejectsAnonymousMeetingCancellationRequest() throws Exception {
        ...
        /* 인증 필터에서 거절된 요청은 취소 유스케이스를 한 번도 호출하면 안 된다. */
