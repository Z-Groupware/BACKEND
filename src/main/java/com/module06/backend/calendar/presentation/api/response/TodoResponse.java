...
public record TodoResponse(Long id, String title, LocalDate date, boolean isDone) {

    public static TodoResponse from(PersonalTodo todo) {
        return new TodoResponse(todo.getId(), todo.getTitle(), todo.getDate(), todo.isDone());
    }
}
