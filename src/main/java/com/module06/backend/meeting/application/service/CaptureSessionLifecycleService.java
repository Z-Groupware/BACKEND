...
public class CaptureSessionLifecycleService implements PauseCaptureSessionUseCase {

    /* 회의 host 조회와 캡처 세션 잠금·저장을 수행하는 D 도메인 저장소다. */
    private final CaptureSessionControlRepository captureSessionControlRepository;

    /* 일시정지 시각과 updatedAt을 동일한 KST 시각으로 기록하기 위한 서버 시계다. */
    ...
    private void validateCommand(PauseCaptureSessionCommand command) {
