...
public interface MeetingSummaryQueryPort {
    ...
    enum SummaryStatus {
        ...
        /*
         * 파이프라인이 끝까지 갔다 — **모든 계층이 DONE 이다.**
         *
         * "실패도 없고 도는 것도 없다"로는 부족하다. 그 조건만 보면 계층 일부만 남은 회의도
         * 완료가 되는데(예전 실행이 계층 사이에서 죽은 경우) 그 회의는 DIST 가 돌지 않았다.
         * 판정 기준은 오케스트레이터의 {@code isFullyAnalyzed} 와 같은 집합이다.
         *
         * <h2>액션이 하달됐다는 뜻이다</h2>
        ...
        /*
         * 중단 — 분석이 도중에 멈췄고 지금 아무것도 돌지 않는다. 두 모양이 여기 들어온다.
         *
         *   ① 계층이 RUNNING 인데 그 실행이 죽었다(배포·크래시 · #177)
         *   ② 계층 사이에서 죽어 **RUNNING 행조차 없다** — 앞 계층은 DONE, 뒤 계층은 행이 없다
         *
         * <h2>⚠ 재시도 버튼은 재실행(ANLZ-01)으로 걸어야 한다</h2>
         * ①은 재개(ANLZ-02)로 이어진다. **②는 재개가 거절된다** — 자동 재개 지점을 고르는
         * {@code firstBrokenLayer} 가 "행이 있고 FAILED 이거나 멈춘" 계층만 깨진 것으로 보므로,
         * 행이 아예 없는 계층은 후보가 아니고 ANLZ-008(재개할 자리가 없음)이 난다.
         *
         * 재실행(ANLZ-01)은 둘 다 열린다 — 모든 계층이 DONE 이 아니면 "이미 완료" 생략을
         * 지나지 않는다. 그래서 화면의 재시도는 재실행이어야 한다.
         */
        STALLED,
