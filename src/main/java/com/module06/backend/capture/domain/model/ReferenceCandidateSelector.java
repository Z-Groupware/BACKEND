...
public final class ReferenceCandidateSelector {
     ...
     * 사람을 가리키는 표현(PERSON).
     *
     * ⚠ 맨 "저"를 넣지 않는다 — "저장", "저번"에 걸려 거의 모든 발화가 후보가 된다.
     * 그러면 좁히는 의미가 사라진다. 대신 조사가 붙은 형태를 나열한다.
     *
     * <h3>격을 빠짐없이 적는다 — 여기가 이 클래스에서 가장 촘촘해야 하는 자리다</h3>
     * 1인칭은 L4 의 담당자 판정이 직접 읽는다("제가 할게요" → 담당자). 그리고 조사 하나를
     * 빠뜨렸을 때의 손실이 다른 종류보다 크다 — **폴백이 구해주지 않는다.** Python 은
     * targets 가 **완전히 비었을 때만** 전체로 되돌리므로(`return narrowed or ...`), 같은
     * 회의의 다른 발화가 하나라도 걸리면 놓친 발화는 그대로 사라진다. 그래서 주격만 적고
     * 여격·목적격을 빼면 "저한테 주세요"의 화자가 조용히 안 풀린다.
     *
     * 반말도 넣는다("내가 할게"). 동료끼리 녹음된 회의에서 흔하고, 여기서 빠지면 그 회의의
     * 담당자가 통째로 미정이 된다. 맨 "내 "는 넣지 않는다 — "사내 규정", "국내 영업",
     * "안내 메일"이 업무 회의에서 자주 나온다.
     *
     * <h3>호칭은 맨 "님"으로 잡는다</h3>
     * 조사를 나열하다 보면 "님도"·"님을"·"님하고"를 계속 빠뜨린다. "님"은 한국어에서 거의
     * 전적으로 사람을 높이는 접미사이므로, 넓혀도 잡히는 것이 실제로 사람이다.
     *
     * "씨"는 그렇게 못 한다 — "날씨"·"씨앗"에 걸린다. 조사 붙은 형태로 나열한다.
     */
    private static final List<String> PERSON_MARKERS = List.of(
            // 1인칭 존댓말 — 주격·소유·여격·목적격·공동격
            "제가", "제 ", "제게", "저는", "저도", "저희", "저의",
            "저에게", "저한테", "저를", "저랑", "저와",
            // 1인칭 반말
            "내가", "나는", "나도", "나에게", "나한테",
