...
/* comment.
    project(C)와 storage(F, 김현지) 사이의 경계. 업로드 URL 발급·다운로드 URL 발급·오브젝트
    삭제를 다룬다. CapObjectStoragePort와 동일 패턴 — s3Key는 호출자가 companyId·projectId로
    미리 조립해서 넘긴다. 이 Port·구현체는 완성된 키만 다루고 네이밍 규칙은 모른다.
    ...
    버킷이 비공개라 deleteObject·issueDownloadUrl도 같은 값을 그대로 받아 키로 쓴다.

    issueDownloadUrl(2026-08-10, 이홍근 요청 — 업로드는 되는데 다운로드할 방법이 없던 구멍)은
    CapS3ObjectStorageAdapter.issuePlaybackUrl과 동일한 presigned GET 패턴이다.
...
public interface ProjectAttachmentStoragePort {
    ...
    IssuedDownloadUrl issueDownloadUrl(String s3Key);
    ...
    record IssuedDownloadUrl(String downloadUrl, int expiresInSeconds) {
    }
}
