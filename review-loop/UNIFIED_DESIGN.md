...
## 9. 최적화 루프 — 루프가 자기 자신을 측정한다 (2026-08-12)

§5의 학습 루프는 **판정 품질**을 개선한다(오탐이 줄어든다). 그런데 **루프 자체가 잘 돌고 있는지**를
답하는 것은 없었다. 계측이 전부 코드를 향해 있었기 때문이다:

| 기존 계측 | 측정 대상 |
|---|---|
| `ReviewReport` | 라운드 나열 · 판정 미수행 건수 (**사건 기록**) |
| `reviewAccuracy` | 규칙별 오탐률 (**규칙 품질**) |
| `review-score-domains.sh` | 도메인 점수 · churn 교차 (**코드 품질**) |

빠진 축이 **루프 자신**이다. 판정 1회가 얼마나 신호를 내는지, 변경이 리뷰를 얼마나 받는지,
나온 신호가 학습으로 얼마나 전환되는지 — 측정하지 않는 것은 개선되지 않는다.

### 지표 세 개 (`LoopMetrics` · `./gradlew reviewOptimize`)

| 지표 | 정의 | 기준 | 미달이면 돌릴 손잡이 |
|---|---|---|---|
| **커버리지** | 리뷰된 `.java` ÷ (리뷰된 + 리뷰되지 않은) | ≥90% | 키 주입(`SETUP.md` §2)·쿼터 — 소급 리뷰 |
| **판정 수율** | finding을 낸 판정 ÷ 전체 판정 | ≥20% | 대상 좁히기(core 파일 유형) 또는 judge 규칙 추가 |
| **학습 전환율** | 사람이 판정해 **교훈으로 기록한** finding ÷ 전체 finding | ≥50% | `DRIVER.md` 8번(`reviewLesson` 1줄) |

전환율이 판정만이 아니라 **기록까지**를 세는 이유: 판정은 사람 머릿속에 남고 프롬프트에는 실리지 않는다.
`lessons.jsonl`에 들어가야 다음 판정이 그 교훈을 읽는다 — 기록되지 않은 판정은 루프를 닫지 않는다.

기준에 미달한 지표는 **반드시 조치와 함께** 출력된다. 숫자만 보여주는 리포트는 아무것도 바꾸지 않는다.
`--snapshot`이면 `knowledge/loop-metrics.jsonl`에 원시 카운터를 한 줄 적재하고, 다음 실행이 '이전' 칸에
그 값을 쓴 뒤 **`변화` 칸에 차이를 %p로 계산해** 보여준다(▲ 개선 · ▼ 악화). 두 숫자를 나란히 놓기만 하면
읽는 사람이 매번 빼야 하고, 그러면 '좋아지고 있나'가 답이 되지 않는다. 이 적재가 있어서 한 번 보는
리포트가 아니라 루프가 된다.

**자동 튜닝은 하지 않는다.** 불변식(고치는 주체는 사람·드라이버)과 같은 이유다. 지표는 손잡이를 지목하고,
돌리는 것은 사람이다.

### 첫 측정 (2026-08-12 · 감사 로그 163행 · 팀 누적)

| 지표 | 값 | 판정 |
|---|---|---|
| 커버리지 | **19%** (리뷰된 119 / 리뷰되지 않은 512) | ❶ 생략 44회 — 키 미주입 41 · 429 3 |
| 판정 수율 | **7%** (119회 중 8회만 신호) | ❷ LLM 호출 대부분이 빈손 |
| 학습 전환율 | **13%** (finding 8건 중 1건 기록) | ❸ 신호가 기억으로 남지 않음 |
| 임계 미달 판정 | **0건** | ❹ **재수정 라운드가 한 번도 발동하지 않았다** |

세 지표가 전부 기준 미달이고, 그 사실이 **여태 어디에도 드러나지 않았다.** 게이트는 매번 초록이었다 —
§8의 "초록불이 두 가지 뜻을 갖는다"가 개별 실행에서 이미 지적된 문제인데, **누적으로 보면
19%였다**는 것이 이 지표의 첫 수확이다. 가장 비싼 것은 커버리지다(리뷰 없이 나간 변경 512개).

**❹는 지표를 붙이고서야 보인 것이다.** 감점 15 · 임계 80이면 단일 지적은 `100-15=85`로 항상 PASS다 —
`NEEDS_REVISION`에 도달하려면 **한 파일에 finding 2건**이 필요하다. 즉 DRIVER.md 3~7번(예산·수정·검증·재판정)은
지금까지 게이트 때문에 돌아본 적이 없고, 실제로 쓰인 것은 **요청서 생성**뿐이다. §8이 "Gate 2 = 리포터"로
확정한 것과 방향은 같지만, 그건 *차단*에 대한 결정이었고 이건 *수정 루프 발동*이라 다른 층이다.
정책으로 유지하든(요청서만) 임계를 조정하든(`meta.score` — yaml SSOT라 코드 수정 없음) **선택은 사람 몫**이다.
지표는 그 선택이 필요하다는 사실만 드러낸다.

### 일부러 계산하지 않는 것 — '수렴 라운드'

`AuditRecord.round`는 **기록한 러너에 따라 뜻이 다르다.** `AutoFixRunner`에서는 진짜 수렴 라운드지만
현 주 경로인 `ReviewLoopRunner`에서는 **한 실행 안의 파일 순번**이다(`++round`가 파일 루프 안에 있다).
둘이 한 파일에 섞여 있으므로 "평균 몇 라운드에 수렴하나"는 지금 데이터로 낼 수 없다.

계산하면 그럴듯한 숫자가 나오고, 그 숫자로 **예산 캡(AutoFix ≤3 · Total ≤6)을 조정**하게 된다 —
잘못된 계측에 기반한 최적화는 계측 없는 상태보다 나쁘다. 필요해지면 `AuditRecord`에 "무엇을 세는
round인지" 구분 필드를 넣는 것이 **선행 조건**이다. `LoopMetricsTest`가 이 문구를 출력에 고정해,
누군가 round 평균을 수렴 라운드로 계산하면 테스트가 깨진다.

같은 이유로 **규칙별 수율도 내지 않는다** — 감사 로그 행에 `ruleId`가 없다. 규칙 단위 신호는
`lessons.jsonl`과 `logs/fix-trail.jsonl`에만 있다.

### 판독 공유

`ReviewReport.records()`로 감사 로그 판독(중복 제거 + 시각순)을 뽑아 `LoopMetrics`가 재사용한다.
각자 파싱하면 union 중복을 한쪽만 걸러도 **같은 파일에서 다른 숫자**가 나오고, 그러면 리포트와 지표가
어긋나 둘 다 신뢰를 잃는다. 루프 자신의 규칙 CONV_001(재발명 금지)이 가리키는 자리다.
