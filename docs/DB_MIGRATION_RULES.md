...
> 기준: 첨부 ERD(Module06 Ver 1.1.0)를 **V1 baseline**으로 고정, 이후 담당자별 메이저 버전(`V2`~`V7`)
...
- 이후 모든 신규 DB 변경은 **담당자별 메이저 버전 영역**에서 증분으로만 추가한다.
...
각 담당자는 **배정받은 영역만** 사용한다. 버전은 **3단계** `V{담당자}.{묶음}.{순번}` 이다.

| 담당자 | 버전 영역 | 예시 |
| --- | --- | --- |
| 윤종호 | `V2.x.y` | `V2.2.2__...sql` (아래 주의 참조) |
| 모성진 | `V3.x.y` | `V3.1.1__...sql` |
| 김현지 | `V4.x.y` | `V4.1.1__...sql` |
| 이태연 | `V5.x.y` | `V5.1.1__...sql` |
| 김민섭 | `V6.x.y` | `V6.1.1__...sql` |
| 박종준 | `V7.x.y` | `V7.1.1__...sql` |

> Flyway는 동일 버전 번호를 허용하지 않으므로, 영역 분리가 파일명/버전 충돌을 원천 차단한다.

**순번 매기는 법** — 같은 작업 묶음 안에서는 마지막 자리를 1씩 올리고, 작업 묶음이 바뀌면 가운데 자리를 올려 마지막을 1로 되돌린다.

```text
V2.2.1 → V2.2.2 → V2.2.3 → ... → V2.2.9   ← 한 묶음(계정 라이프사이클 기반 스키마)
                                  V2.3.1   ← 다음 묶음부터 가운데 자리 증가
```

Flyway는 각 자리를 숫자로 비교하므로 `V2.2.9 < V2.3.1` 이고 `V2.2.9 < V2.2.10` 이다. 자리수 걱정 없이 계속 올려도 순서가 어긋나지 않는다.

**`V1`은 baseline(`V1__init_schema.sql`) 전용이며 누구에게도 배정하지 않는다.** 담당자 레인이 `V2`부터 시작하고 박종준이 `V7`인 이유다.

**이미 머지된 파일은 이름을 바꾸지 않는다.** 파일명을 바꾸면 이력상 "미적용 신규 파일"이 되어 재실행되거나 validate가 실패한다(3번 참조).

> ⚠️ **윤종호 레인은 `V2.2.2`부터 이어 쓴다.** `V2.2.1__add_is_admin_to_member.sql`이 이미 적용돼 있어, 그보다 낮은 번호(`V2.1.x`·`V2.2.0`)를 새로 만들면 적용된 버전보다 낮아져 out-of-order 상황이 된다(5번 참조).
...
- ❌ 병합된 `V3.1.1__...sql` 내용 변경
- ✅ 새 파일 `V3.1.2__...sql`로 보완
...
```text
적용 순서:  V3.1 → V4.1 → V3.2 (뒤늦게 병합, 지금 실행됨) → V5.1
버전 순서:  V3.1 → V3.2 → V4.1 → V5.1                        ← 이렇게 되지 않는다
```

> **그래서 지켜야 할 규칙: 낮은 버전 마이그레이션이 높은 버전의 변경에 의존하면 안 된다.**
> 예) `V3.2`가 `V4.1`이 추가한 컬럼을 참조하면, 위 순서에서는 우연히 성공하지만
> 빈 DB에 처음부터 적용할 때는 버전 순서대로 실행되므로 **실패한다.**
> 새 컬럼을 참조하는 변경은 반드시 자기 버전보다 **낮은** 버전에만 의존하도록 작성한다.
> 담당자당 메이저 버전 1개 체계에서는 **레인 번호가 낮은 담당자일수록** 이 제약을 더 자주 만난다(예: `V2` 레인은 항상 `V3`~`V7`보다 낮다).
...
```text
[DB 변경 예정]
파일명: V2.3.1__add_last_login_at_to_member.sql
대상: member
...
2. Flyway 초기화
3. 애플리케이션 재실행
4. **`V1`(baseline) → 담당자 레인(`V2.x.y`~`V7.x.y`)** 순서로 재적용
...
- 파일명: `V{버전}__{설명}.sql` (설명은 영문 snake_case)
  예) `V4.1.2__add_capacity_index_to_meeting_room.sql`
...
- ⚠️ **`R__` 는 checksum 이 바뀔 때마다 다시 실행된다.**
  ...
  - ✅ **일회성 시드는 versioned(자기 레인 `V{N}.x.y`)로** 작성한다.
...
- [ ] 담당 버전 영역(자기 메이저 버전 `V{N}.x.y`)만 사용
- [ ] 공용 테이블 변경 시 사전 공유
